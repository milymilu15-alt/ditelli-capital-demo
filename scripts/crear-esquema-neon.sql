-- ===================================================================
-- Ditelli Capital — CREAR EL ESQUEMA DESDE CERO
--
-- El error 42P01 "relation members does not exist" significa que esta
-- base no tiene NINGUNA de las tablas: nunca se le corrio schema.sql.
-- Los ALTER del archivo anterior no servian porque la tabla que
-- intentaban modificar todavia no existia.
--
-- Este archivo es el schema.sql completo. No borra ni pisa nada: todos
-- los CREATE son IF NOT EXISTS y todos los ALTER son ADD COLUMN IF NOT
-- EXISTS. Es seguro correrlo entero, y seguro correrlo dos veces.
-- ===================================================================


-- -------------------------------------------------------------------
-- PASO 0 — ANTES DE CORRER NADA: confirmar que estas en la base correcta
--
-- Neon puede tener varias branches y varias bases. Si corres esto en una
-- y Railway apunta a otra, el error va a seguir igual.
-- Corre SOLO esta linea primero y compara el resultado con el nombre de
-- base que aparece al final del DATABASE_URL cargado en Railway
-- (.../NOMBRE_DE_BASE?sslmode=...). Tienen que coincidir.
-- -------------------------------------------------------------------
SELECT current_database() AS base_actual, current_user AS usuario;


-- -------------------------------------------------------------------
-- PASO 1 — ESQUEMA COMPLETO (todo lo de abajo es schema.sql tal cual)
-- -------------------------------------------------------------------
...
-- -------------------------------------------------------------------
-- PASO 2 — VERIFICACION
-- Tienen que aparecer las 5 tablas:
-- admin_users, magic_links, member_envelopes, members, membership_requests
-- -------------------------------------------------------------------
SELECT table_name
...
-- Y esta tiene que devolver 0 filas (ninguna columna faltante):
SELECT esperada.col AS columna_que_falta
FROM (VALUES
    ('id'),('public_token'),('name'),('doc_id'),('email'),('phone'),
    ('amount_usd'),('status'),('proportional_pct'),('envelope_id'),
    ('mp_payment_id'),('stripe_payment_id'),('mp_preference_id'),
    ('stripe_session_id'),('amount_ars'),('fx_rate_ars_per_usd'),
    ('signed_pdf_size_bytes'),('signed_pdf_url'),('transfer_receipt_data'),
    ('transfer_receipt_content_type'),('transfer_receipt_filename'),
    ('transfer_receipt_uploaded_at'),('transfer_confirmed_by'),
    ('transfer_confirmed_at'),('is_test'),('created_at'),('updated_at')
) AS esperada(col)
